...
"""
Legacy helper functions for JUMP Validator.

This file originates from a Jupyter notebook export and contains many utility
functions used by the validator pipeline. The goal of the edits in this repo is
*not* to change the underlying algorithms, but to:

- improve cross-platform path handling (macOS/Linux/Windows),
- make directory creation robust,
- add lightweight documentation where helpful.

Most functions are kept as-is to preserve output compatibility.
"""
...
#intensity normalization validation
def normalize_intensity(intensity, maxval): #intensity is list of intensity
    # maxval = np.max(intensity)
    intensity = list(map(float, intensity))
    normTest = np.array(intensity)/maxval*100
    return list(normTest)
...
def prepareInput(inputFile):
    ...
    # ------------------------------------------------------------------
    # Cross-platform robustness / workflow improvement (no algorithm change)
    # ------------------------------------------------------------------
    # Scan/charge columns are sometimes parsed as floats (e.g., 12345.0),
    # especially when the file comes from Excel or contains missing values.
    # Using `.astype(str)` preserves the `.0` suffix, which can prevent spectrum
    # matching later (leading to empty results).
    #
    # We normalize "integer-like" values to clean strings while leaving
    # non-integer values untouched.
    import re

    def _intlike_to_str(v):
        if pd.isna(v):
            return ""
        # Numeric types
        try:
    ...
    df["scan"] = df["scan"].apply(_intlike_to_str)
    df["charge"] = df["charge"].apply(_intlike_to_str)
...
def mkdir(dir1):
  ...
  The legacy implementation used `os.system("mkdir ...")`, which is
  platform-dependent and can emit noisy errors when the directory already
  exists. `os.makedirs(..., exist_ok=True)` is cross-platform and quiet.
  """
  try:
    Path(dir1).mkdir(parents=True, exist_ok=True)
  except Exception:
    # Preserve legacy behavior: do not raise if the directory cannot be created.
    pass
...
def modsForReport2(mods, peptide):
  aa_list = []
  pos_list = []
  mass_list = []
  modSplit = mods.split(",")
  for x in modSplit:
    ...
    mass_list.append(str(xSplit[2]))
  return aa_list, pos_list, mass_list
...
def excelWriter3(writer, df,  worksheetName, figure1, spectrumSplit,xcorr, massSeriesLength, peptide, updater=0, index_sp=0):

  nupdate = updater+1
  spectrum_list = spectrumSplit[0].split(".")
  run = spectrum_list[0]
  scan = spectrum_list[1]
  charge = spectrum_list[2]
  peptide = spectrumSplit[1]

  aa_list, pos_list, mass_list= modsForReport2(spectrumSplit[2], peptide)

  #after knowning # of modifications we can add figure by 4*noof mods cells addition
  mods_cell_add = len(aa_list)*4
  positionDF = updater+10+mods_cell_add+13
  df.to_excel(writer, sheet_name=worksheetName, startrow=positionDF, startcol = 1, index=None)
  workbook  = writer.book
  worksheet = writer.sheets[worksheetName]

  text1 = "Spectrum number: "
  text11 = str(index_sp+1)

  bold = workbook.add_format({'bold': True})
  worksheet.write_rich_string('B{}'.format(nupdate),bold,text1, text11) 
  
  nupdate+=1
  text2 = "Run: "
  text22 = run

  worksheet.write_rich_string('B{}'.format(nupdate),bold,text2, text22)

  nupdate+=1
  text3 = "Scan: "
  text33 = str(scan)

  worksheet.write_rich_string('B{}'.format(nupdate),bold,text3, text33)

  nupdate+=1
  text4 = "Charge: "
  text44 = str(charge)
  
  worksheet.write_rich_string('B{}'.format(nupdate),bold,text4, text44)
    
  nupdate+=1
  text5 = "Peptide sequence:"
  text55 = peptide

  worksheet.write_rich_string('B{}'.format(nupdate),bold,text5, text55)
  
  for val in range(0,len(aa_list)):
    var1 = "Modification #{}: ".format(val+1)
    nupdate+=1
    worksheet.write('B{}'.format(nupdate),var1, bold)

    nupdate+=1
    var2 = "Residual amino acid: {}".format(aa_list[val])
    worksheet.write('C{}'.format(nupdate),var2)
    
    nupdate+=1
    var3 = "Residual position: {}".format(pos_list[val])
    worksheet.write('C{}'.format(nupdate),var3)

    nupdate+=1
    var4 = "Mass: {}".format(mass_list[val])
    worksheet.write('C{}'.format(nupdate),var4)
    
  nupdate+=1
  text6 = "Peptide identification score: " 
  text66 = "{} (JUMP Jscore)".format(xcorr)
  worksheet.write_rich_string('B{}'.format(nupdate),bold,text6,text66)
  
  nupdate+=1
  text7 = "Figure A."
  text77 = " MS/MS Spectrum of the identified peptide (matched ions highlighted in red)"
  worksheet.write_rich_string('B{}'.format(nupdate),bold,text7, text77)
  
  nupdate+=2
  location = "B"+str(nupdate)
  ...
  text8 = "Figure B."
  text88 = " b and y ion series with different charge states"
  worksheet.write_rich_string('B{}'.format(nupdate),bold,text8, text88)

 
  # massSeriesLength = df.shape[0]
  updater = nupdate+massSeriesLength+2
...
def mzmlDF_SingleFile(mzxmlFile, spec_list):
  ...
  run = os.path.basename(mzxmlFile).split(".mzXML")[0]
...
#         ms2DF=msToDF(mzXML_list, list(set(df_pho["spectrum"])))
        ...
        run = os.path.basename(mzxmlFile).split(".ms2")[0]
    ...
    #     ori_peptide = row[mz_cols.index("Peptides")+1]
        ...
        maxCharge = int(row[mz_cols.index("charge")+1])+1
...
def createOutfile(row, df):
  ...
  if "Outfile" in columns:
    ...
    #print (outfile_split)
    # NOTE: The legacy notebook used string splitting on "/" which fails on
    # Windows-style paths embedded in files produced elsewhere. We normalize
    # path separators before taking the basename. This does not change behavior
    # for POSIX paths.
    run = os.path.basename(str(outfile_split[-5]).replace("\\", "/"))
...
def ms2ToDf(ms2File):
    ...
    exp = os.path.basename(ms2File)[0:-4]
...
def ms2ToDf_spec(ms2File):
    ...
    #         print ("peptide ID found")
        ...
        elif "L\tID_Decoy" in line:
            temp_line = line.strip().split("\t")
            L_ID_decoy = "Decoy_"+temp_line[2]
            L_ID_list.append(L_ID_decoy)
        elif "L\tPrecursor_Decoy" in line:
            temp_line = line.strip().split("\t")
            charge = temp_line[-2].split("=")[-1]
            prec_mz_decoy = temp_line[-1]
            L_peptide_decoy = "Decoy;"+str(charge)+";"+str(prec_mz_decoy)
            
            L_peptide_list.append(L_peptide_decoy) 
        elif "L\tRT" in line:
    #         print ("RT found")
            temp_line = line.strip().split("\t")
    ...
    dict1 = {"scan":scan_list,"charge":charge_list,"[M+H]+":MH_list,
                "prec_MZ":precursorIon_list,"L_ID":L_ID_list,"L_peptide":L_peptide_list
                     ,"RT":RT_list,"m/z":ms2_mz,"intensity":ms2_int}
    ...
    exp = os.path.basename(ms2File).split(".ms2")[0]
    ms2Df["spectrum"] = exp+"."+ms2Df.scan.astype("str")+"."+ms2Df.charge.astype("str")
    return ms2Df
...
def mzmlDF(mzXML_list, spec_list):
  ...
  df = pd.DataFrame([x for x in x1])  #dataframe of the mzXML file
  ms2 = df.copy().loc[df.msLevel==2]     #ms2 level scans
  run = os.path.basename(mzXML_list[0]).split(".mzXML")[0]
  ms2["expScan"] = run+"."+ms2.num.astype("str")
  ms2F = ms2.copy().loc[ms2.expScan.isin(spec_list)]
  ms2F.rename(columns={"num":"scan","m/z array":"m/z","intensity array":"intensity"}, inplace=True)
  reqCols = ["expScan","scan","m/z","intensity"]
  ...
  for x in range(1,len(mzXML_list)):
    ...
    run2 = os.path.basename(mzXML_list[x]).split(".mzXML")[0]
...
def msToDF(allms2Folder, spec_list):
  df = ms2ToDf_spec(allms2Folder[0])
  df_pre3 = df.copy().loc[df.spectrum.isin(spec_list)]
  run = os.path.basename(allms2Folder[0]).split(".ms2")[0]
  ...
  for x in range(1,len(allms2Folder)):
    df_2 = ms2ToDf_spec(allms2Folder[x])
    df_pre33 = df_2.copy().loc[df_2.spectrum.isin(spec_list)]
    run2 = os.path.basename(allms2Folder[x]).split(".ms2")[0]
